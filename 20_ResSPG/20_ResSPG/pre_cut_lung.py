...
def get_segmented_lungs(img_dir, plot=False, min_lung_area=3000, padding_num=0):
    ...
    src_img = cv2.imdecode(np.fromfile(img_dir, dtype=np.uint8),0)
    src_img = np.expand_dims(src_img, axis=2)
    src_img = np.concatenate((src_img, src_img, src_img), axis=-1)
    ...
    if img.mean() < 50 or img.mean() > 180:
        return (False, [], [])
    img = img * 3
    img -= 1000
    im = img.copy()
...
def make_save(save_path):
    ...
    source_crop_dir = os.path.join(save_path, 'source_crop')
    os.makedirs(source_crop_dir, exist_ok=True)
    process_dir = os.path.join(save_path, 'filled_crop')
    os.makedirs(process_dir, exist_ok=True)
    return source_dir, crop_dir, source_crop_dir, process_dir
...
def pre_deal(src_img_dir, save_img_dir):
    ...
    for item in img_list[choose_index]:
        ...
        Flag, filled_crop_img, crop, crop_source = get_segmented_lungs(img_dir)
        if Flag:
            shutil.copy(img_dir, s1_dir)
            cv2.imwrite(s2_dir, crop)
            cv2.imwrite(s3_dir, crop_source)
            cv2.imwrite(s4_dir, filled_crop_img)
...
if __name__ == '__main__':
    ...
    for people in os.listdir(fp):
        try:
            pre_deal(f'{fp}/{people}', ft)
        except:
            print(people)
