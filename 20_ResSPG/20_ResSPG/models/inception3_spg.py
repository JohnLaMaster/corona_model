...
def model(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        ckpt = model_zoo.load_url(model_urls['inception_v3_google'])
        for module_name in list(model._modules):
            tmp_dic = OrderedDict({item.replace(module_name+'.', ''):ckpt[item] for item in list(ckpt.keys()) if module_name in item})
            if len(tmp_dic) == 0:
                continue
