...
from utils.anchors_FPN import get_anchor_fpn
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, threshold=0.7, 
        ...
        self.classier_1 = nn.Conv2d(
                1024, num_classes, kernel_size=1, padding=0)  #fc8
        #------------------------------------------
        ...
        self.edge_anchors = get_anchor_fpn()
...
# =============================================================================
    ...
    def get_loss(self, logits, gt_labels):
        ...
        #print(side4.size(),torch.squeeze(torch.sigmoid(self.interp(side4)),dim=1).size(),side4.size(),self.interp(side4).size())
        mask_side4 = self.get_mask(mask_side4, torch.squeeze(torch.sigmoid(self.interp(side4)),dim=1), 0.5, 0.05)
        ...
        back_mask = self.get_mask(back_mask, torch.squeeze(fused_atten.detach(),dim=1), 0.7, 0.1)
...
class ProposalNet(nn.Module):
    def __init__(self):
        ...
        self.ReLU = nn.ReLU()
        self.tidy1 = nn.Conv2d(128, 1024, 1, 1, 0)
        self.tidy2 = nn.Conv2d(128, 1024, 1, 1, 0)
        self.tidy3 = nn.Conv2d(128, 1024, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        t1 = self.tidy1(d1).view(batch_size, -1, 1024)
        t2 = self.tidy2(d2).view(batch_size, -1, 1024)
        t3 = self.tidy3(d3).view(batch_size, -1, 1024)
